...
namespace VoloBlog.Migrator
{
    class Program
    {
        ...
        {
            Console.WriteLine("Initializing VoloBlog Migrator ... ");

            using (var app = AbpApplicationFactory.Create<VoloBlogMigratorModule>())
            {
                app.Initialize();

                using (var dbContext = app.Resolve<VoloBlogDbContext>())
                {
        ...
        private static void RunMigrations(string connectionString, VoloBlogDbContext dbContext)
        {
        ...
        private static void GenerateMigrationScript(VoloBlogDbContext dbContext)
        {
